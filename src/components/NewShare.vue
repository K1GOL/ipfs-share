<template>
  ...
  <div :class="dynamicBlur" class="w-full h-full bg-gradient-to-br from-slate-700 to-slate-800 flex flex-col items-center justify-center text-zinc-100 border-zinc-100 transition duration-150 ease-in-out  z-0">
    ...
    <p class="p-8">Powered by the <a href="https://ipfs.io/" target="_blank" class="underline decoration-solid hover:decoration-double"> Interplanetary File System</a></p>
  </div>
  ...
  <v-dialog :class="dynamicOpacity" v-if="showDialog" v-model="showDialog" class="w-screen h-screen flex place-content-center place-items-center fixed transition duration-150 ease-in-out text-zinc-100 z-10">
    ...
    <button class="place-self-end p-4 pb-0" @click="hideDialog">X</button>
      ...
      <div class="w-full">
        <p class="p-4 max-w-full inline-block underline decoration-solid hover:decoration-double truncate"><a class="max-w-90 truncate" :href="'https://ipfs.io/ipfs/' + fileCid" target="_blank">https://ipfs.io/ipfs/{{ fileCid }}</a></p><br>
        <button @click="copyToClipboard" class="p-4 hover:underline hover:decoration-solid">Copy to clipboard</button>
      </div> 
    </div>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    upload (event) {
      ...
      if (event.target.files[0]) {
        ...
        loadScript('https://cdn.jsdelivr.net/npm/ipfs/dist/index.min.js').then(async () => {
          ...
          const results = await node.add(event.target.files[0]);
          if (results) {
            // The file was successfully uploaded to the IPFS for all to enjoy, retrieve ID:
    ...
    copyToClipboard () {
      navigator.clipboard.writeText(`https://ipfs.io/ipfs/${this.fileCid}`);
